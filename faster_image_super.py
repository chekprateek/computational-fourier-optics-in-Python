...
# -*- coding: utf-8 -*-
"""
Created on Sat Jan 18 12:06:26 2025
...
# Precompute circular aperture
circ_aperture = circ(np.sqrt(Fu**2 + Fv**2) * 2 * lambda_ * fnum)

# Loop through image plane positions
for n in range(M):
    v0 = (n - (M/2 + 1)) / (M/2)  # norm v image coord
    print(f"Processing row {n+1}/{M}", end='\r')  # Simple progress indicator
    for m in range(M):
...
print("\nProcessing complete.")  # Indicate completion
